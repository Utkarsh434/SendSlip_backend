package com.ansh.sendslip.security;
...
public class ClerkJwksProvider {
    ...
    private PublicKey createPublicKey(String modulus, String exponent) throws Exception{
        byte[] moduludBytes = Base64.getUrlDecoder().decode(modulus);
        byte[] exponentBytes = Base64.getDecoder().decode(exponent);

        BigInteger moduleBigInt  = new BigInteger(1,moduludBytes);
        BigInteger exponentBigInt = new BigInteger(1,exponentBytes);

        RSAPublicKeySpec spec = new RSAPublicKeySpec(moduleBigInt,exponentBigInt);
